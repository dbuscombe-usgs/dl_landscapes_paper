...
def norm_im(image_path, tile):
   ...
   image_reader = tf.image.decode_jpeg(file_reader, channels = 3,
                                        name='jpeg_reader')
   float_caster = tf.image.resize_images(tf.cast(image_reader, tf.float32), [tile,tile])
...
def eval_tiles(label, direc, numero, classifier_file, x, tile):
   graph = load_graph(classifier_file)
...
if __name__ == '__main__':
   ...
   classifier_file = 'nwpu_mobilenetv2_96_1000_001.pb'
   #classifier_file = 'nwpu_mobilenetv2_224_1000_001.pb'
   ...
   plt.savefig('nwpu_cm_96.png', dpi=300, bbox_inches='tight')
   del fig; plt.close()
