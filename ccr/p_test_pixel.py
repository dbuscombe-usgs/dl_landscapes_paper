...
if __name__ == '__main__':
   ...
   classifier_file = 'ccr_mobilenetv2_'+str(tile)+'_1000_001.pb'
   ...
   tmp = [i for i, x in enumerate([x.startswith('surf') for x in labels]) if x].pop()
   cmap1[tmp] = 'w'

   tmp = [i for i, x in enumerate([x.startswith('water') for x in labels]) if x].pop()
   cmap1[tmp] = 'b'

   tmp = [i for i, x in enumerate([x.startswith('beach') for x in labels]) if x].pop()
   cmap1[tmp] = 'y'

   tmp = [i for i, x in enumerate([x.startswith('buildings') for x in labels]) if x].pop()
   cmap1[tmp] = 'r'
   ...
   tmp = [i for i, x in enumerate([x.startswith('cliff') for x in labels]) if x].pop()
   cmap1[tmp] = '#8B008B'
